...
const navItems = [
  ...
  { href: '/courses', label: 'My Tomes', icon: BookOpen },
    ...
    <div className="min-h-screen flex" style={{ background: 'var(--astra-deep)' }}>
      ...
      <>
        ...
        <motion.aside
          ...
          className="fixed top-0 left-0 bottom-0 z-30 w-64 flex-shrink-0 flex flex-col lg:translate-x-0 lg:static lg:z-auto"
          style={{
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 pb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="font-fantasy text-sm font-bold text-gold-gradient">ASTRA</span>
            </div>
            <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-yellow-600/60 hover:text-yellow-500">
              <X className="w-5 h-5" />
            </button>
          </div>
                ...
                <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}>
                  ...
                  >
                    <Icon className="w-4 h-4 shrink-0 transition-colors"
                      style={{ color: isActive ? 'var(--astra-gold)' : 'var(--astra-text-dim)' }} />
                    <span className="text-sm font-medium transition-colors"
                      style={{ color: isActive ? 'var(--astra-text)' : 'var(--astra-text-muted)' }}>
                      {item.label}
                    </span>
                    {isActive && (
              ...
              <>
                ...
                <Link href="/admin" onClick={() => setSidebarOpen(false)}>
                  <motion.div
                    ...
                    style={{
                      background: pathname.startsWith('/admin') ? 'rgba(201,162,39,0.12)' : 'transparent',
                      border: pathname.startsWith('/admin') ? '1px solid rgba(201,162,39,0.2)' : '1px solid transparent',
                    }}
                  >
                    <Crown className="w-4 h-4" style={{ color: pathname.startsWith('/admin') ? 'var(--astra-gold)' : 'var(--astra-text-dim)' }} />
                    <span className="text-sm font-medium" style={{ color: pathname.startsWith('/admin') ? 'var(--astra-text)' : 'var(--astra-text-muted)' }}>
                      Admin Panel
                    </span>
                  </motion.div>
                </Link>
        ...
        >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}>
                  ...
                  >
                    <Icon className="w-4 h-4" style={{ color: isActive ? 'var(--astra-gold)' : 'var(--astra-text-dim)' }} />
                    <span className="text-sm font-medium" style={{ color: isActive ? 'var(--astra-text)' : 'var(--astra-text-muted)' }}>
                      {item.label}
                    </span>
                    {isActive && <ChevronRight className="w-3 h-3 ml-auto" style={{ color: 'var(--astra-gold)' }} />}
                  </motion.div>
                </Link>
              );
            })}
          ...
          <div className="p-4">
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all hover:bg-red-500/10 group"
            >
              <LogOut className="w-4 h-4 group-hover:text-red-400 transition-colors" style={{ color: 'var(--astra-text-dim)' }} />
              <span className="text-sm group-hover:text-red-400 transition-colors" style={{ color: 'var(--astra-text-muted)' }}>
                {loggingOut ? 'Departing...' : 'Leave Academy'}
              </span>
            </button>
          </div>
